...
4. Report what `next` passed over. `--explain N` returns the higher-ranked items that were not selected, each with its reason — unrefined, waiting on a named dependency, explicitly blocked, or a container whose children carry the work — with actionable reasons before structural ones. Report those alongside the selection, because a selection nobody can audit cannot be picked up by whoever comes next. A container with no children is treated as work by both `next` and `gantt`: it is undecomposed, not a grouping.
